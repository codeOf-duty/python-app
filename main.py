...
def reuploadText():
    if request.method == 'POST':
        ...
        for i in dct_:
            data = data.replace(i,'')
        data = data.replace(' Add to DictionarySuggestionReplace AllIgnore"  >'," ")
...
def freqGenerator():
    if 'loggedin' in session and session['loggedin'] == True:
        f = open("datasets/docxDataset-freq/status.sts", mode='w', encoding='utf-8')
        f.truncate(0)
        f.close()
            ...
            for i in list_:
                ...
                if len(temp) == 2:
                    if temp[1] not in dict_ and temp[0] != word and len(temp[0]) >= 2:
                        dict_[temp[1]] = temp[0]
            if option == "add":
                with open(fp.BloomFilterModel,'a', encoding='utf-8') as inFile2:  # Check Twice Before Proceeding
                    inFile2.write("\n"+word)
                with open(fp.SymspellModel,'a', encoding='utf-8') as inFile1:  # Check Twice Before Proceeding
                    inFile1.write("\n{} {}".format(word, "1"))
            elif option == "remove":
                pass
